...
int input_WI_result(){
	ifstream ifs_p("power.txt");	//受信起電力が入ったファイル
	...
	//読んだファイルを数列に置き換えて出力
	string str;					//文字列クラス
	vector<double> input_p;	//動的配列クラス

	//グリッドの各受信電力
	while (getline(ifs_p, str))
	{
		input_p.push_back(atof(str.c_str()));
	}
	//出力先のファイル
	ofstream fout_p( "received_power.txt" );
	if(!fout_p){
	...
	for(k=0 ; k<MAPSIZE_Z ; k++)
	{
		for(j=0 ; j<MAPSIZE_Y ; j++)
		{
			for(i=0 ; i<MAPSIZE_X ; i++)
			{
				...
				{
					input_p[s]= input_p[s];
					received_power1[i][j][k]= input_p[s];
					s++;
					//cout << received_power[i][j][k]  << endl;
					fout_p << i << "," << j << "," << k << "," << received_power1[i][j][k] << endl;
				}
			}
		}
	}
