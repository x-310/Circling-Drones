...
//ポテンシャルを求める
 double get_pot(int x,int y,int z,int loop){
	...
	//案1　全通信不能座標からの斥力を考慮した斥力項
	//経路に関係しない斥力を考えてしまっていたり重みを大きく下げても局所解回避に効果があまり見られなかった
	//for(int k=0;k<MAPSIZE_Z;k++){
	//	for(int j=0;j<MAPSIZE_Y;j++){
	//		for(int i=0;i<MAPSIZE_X;i++){
...
int main(void){
	...
	for (int i = 0; i < max; i++){
		if(x!=goal[0] || y!=goal[1] || z!=goal[2]){
				...
				for(int b = -1; b <= 1; b++){
					for(int c = -1; c <= 1; c++){
						...
						cout << x+a << " " << y+b << " " << z+c << " " << received_power[x+a][y+b][z+c] << " " << get_pot(x+a,y+b,z+c,loop) << endl;//各近傍の表示
							...
							if(get_pot(x+a,y+b,z+c,loop) < min){ //最低値を更新したら行う
			...
			if(rap > 0){		//loop -1なら目標位置ポテンシャルを反転して同じ箇所で再度行う
				loop = -1;
				rap = rap - 1;
                                path_x[i] = x;
                                path_y[i] = y;
			...
			//fout << x << "," << y << "," << z << "," << received_power[x][y][z] << endl;//csvに受信電力値も載せるとき
			//fout << x << "," << y << "," << z << "," << endl;//csvに座標のみ書くとき
